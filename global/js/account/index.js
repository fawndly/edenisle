/*! Crysandrea - Javascript for account/index - Created at August 24, 2012*/
$(document).ready(function(){
	$('#dropdown-1').appendTo('body');
	$('#change_username').on('click', function(){
		$("#change_username_tip").hide();
	...
	$('#timezone').change(function(){
		var timezone_array = $('#timezone').val().split('.');
		var time_string = "";
		var time_split = "am";
		var new_hour = parseFloat(timezone_array[0])+parseFloat(hours);
		var minutes = parseInt(parseInt(timezone_array[1])+parseInt(mins));

		if(new_hour > 11 || new_hour < 23){
			time_split = "pm";
		...
		$('#auto_timezone').removeClass('suggested').text('Auto-suggest');
		$('#timezone_value').text(new_hour+':'+minutes+time_split);
	})

	var timezone = jstz.determine();
	var time_value = 0;

	for( var prop in jstz.olson.timezones ) {
	    if( jstz.olson.timezones.hasOwnProperty( prop ) ) {
	         if( jstz.olson.timezones[ prop ] === timezone.name() )
	             time_value = prop.split(',');
	    }
	}

	function find_time(){
		if(time_value[1] == 1) {
			time_value[0] = parseInt(time_value[0]);
			if(time_value[0] < 0){
				time_value[0] += 60;
			} else {
				time_value[0] -= 60;
	...
	$('#auto_timezone').click(function(){
		if ($(this).hasClass('suggested')) return false;

		find_time();

		$("#timezone").val(parseFloat(time_value[0]/60)+'.0');
		$("#timezone").trigger('change');

		$(this).addClass('suggested').text('Suggested');
