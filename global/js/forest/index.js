...
$(document).ready(function(){
    ...
    var forest = {
		...
		reload_leaderboards: function(){
			js_hunts = 0;
			$.post("/forest/get_leaderboards", function(msg) {
				$('#leaderboard_container').empty();
				$.each(msg, function(k, obj) {
					var $user = $('<a>', {href: '/forest/museum/' + obj.username.split(' ').join('+'), title: obj.username + '\'s Museum'}).addClass('leaderboard_thumbnail');
					$user.append($('<img>', {src: '/images/avatars/' + obj.user_id + '_headshot.png', width: 56, height: 56, alt: ''}));
					$user.append($('<span>').text('#' + (k + 1)));
					$('#leaderboard_container').append($user);
				});
