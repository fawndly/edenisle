 ...
 */
    ...
    public function index() {
        $this->system->view_data['scripts'][] = '/global/js/forest/index.js?1';
        ...
        $view_data = array(
            ...
            'navigation_header' => 'Sapherna\'s Forest',
        ...
        $nets_owned = $this->db->select('user_items.id, avatar_items.name, avatar_items.item_id, COUNT(1) as total')
            ...
            if($nets_owned->num_rows() > 0):
              ...
              foreach ($nets_for_sale as $net):
                if($this->system->userdata['user_Ores'] >= $net['price']):
              ...
              if (count($affordable_nets) > 0):
                ...
                // else:
                  ...
                  if($forest_user_catches_query->num_rows() > 0):
                    ...
                    $response['message'] = 'You can\'t afford any nets at the moment, but you have '.($forest_user_catches_query->num_rows() > 10 ? 'plenty of' : 'a few').' bugs to sell in your bugpack. Here is a <a href="/forest/bugpack">handy link to your bugpack &rsaquo;</a>';
                    $response['response'] = 'sell_bugs';
                  else:
                    // You should post around the forums to get some palladium and buy a new net!
                    $response['message'] = 'You can\'t afford any nets at the moment, you should <a href="/forum">post around the forums &rsaquo;</a> to get some more ores!';
  ...
  public function tour()
  {
    ...
    $view_data = array(
      ...
      'routes'            => $this->route_navigation,
      'active_url'        => $this->uri->rsegment(2, 0),
      'navigation_header' => 'Sapherna\'s Forest Tour',
    );
 ...
 */
   ...
   */
    public function bugpack() {
        ...
        $insect_list = $this->db->query('
                SELECT forest_user_catches.user_id,
                ...
                ORDER BY forest_insects.exp ASC, amount DESC'
            )->result_array();
        ...
        foreach ($insect_list as $bug) {
            ...
            if ($bug['amount'] > 1)
                $sell_incremenets[$bug['amount']-1] = 'Sell all but x1';
  ...
  public function prehunt_checks($hunter_data = array(), $return_net = FALSE)
  {
    ...
    if (($hunter_data['energy']) < 1)
        die(json_encode(array('error' => 'Not enough energy!', 'error_data' => 'yield', 'error_msg' => 'You need to have more energy to be able to hunt!')));
    ...
    if (count($response) < 1):
      ...
      elseif(mt_rand(1, 85) == 2):
          ...
          $response = (array(
            ...
            'name'        => 'Treasure bag!',
            'value'       => $chest_palla,
            'image'       => '/images/forest/ore_bag.jpg',
            'description' => 'You found a total of <b>'.$chest_palla.'</b> ores!'
          ));
 ...
 */
  ...
  public function sell_bug()
  {
    ...
    $get_insects = $this->db->select('forest_user_catches.*, forest_insects.price')
      ...
      if($get_insects->num_rows() == $amount):
          ...
          $this->load->model('user_engine');
          $this->user_engine->add('user_Ores', $profits);
  ...
  public function sell_all_bugs()
  {
    ...
    public function sell_all_but_one() {
        if (!$hunter_data = $this->_get_hunter_data($this->system->userdata['user_id']))
            redirect('/forest/tour');
        ...
        $insects = $this->db->query('
            SELECT `forest_user_catches`.*,
                   `forest_insects`.`price`,
                   `total`
            FROM `forest_user_catches`
            JOIN `forest_insects` ON `forest_user_catches`.`insect_id` = `forest_insects`.`id`
            LEFT JOIN (
                SELECT `forest_user_catches`.insect_id,
                        COUNT(`forest_user_catches`.`insect_id`) as `total`
                FROM `forest_user_catches`
                WHERE `user_id` = ' . $user_id .'
                GROUP BY `forest_user_catches`.`insect_id`
            ) AS `t` ON `t`.`insect_id` = `forest_user_catches`.`insect_id`
            WHERE `user_id` = ' . $user_id .' AND `total` > 1
            ORDER BY `forest_user_catches`.`insect_id` ASC'
        )->result_array();
        ...
        if (count($insects) > 0) {
            ...
            $insectCounter = array();
            foreach($insects as $insect) {
                if (!isset($insectCounter[$insect['insect_id']]))
                    $insectCounter[$insect['insect_id']] = 1;

                if ($insectCounter[$insect['insect_id']] >= $insect['total'])
                    continue;

                $profits += $insect['price'];
                $insectCounter[$insect['insect_id']] = $insectCounter[$insect['insect_id']] + 1;
            ...
            $response = array('profits' => $profits, 'sold' => array_sum($insectCounter));
        } else {
            $response = array('error' => 'You do not have any insects to sell!');
        }
...
if($hunter_data['berries'] < 1):
$response = array('error' => 'You do not own any berries to snack on');
$hunter_data['berries'] = 0;

    elseif($hunter_data['berries'] >= 1):
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"):
      ...
      if($shop_item_query->num_rows() > 0):
        ...
        if(in_array($shop_item_data['item_id'], array_keys($this->nets))):
          ...
          if($this->system->userdata['user_Ores'] >= $shop_item_data['price']):
            $this->user_engine->remove('user_Ores', $shop_item_data['price']);
  ...
  public function fix_nets()
  {
    ...
    // Basic (visible)
    $broken_basic_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 19 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    ...
    if($amount_of_new_basic_nets > 0){
      $this->user_engine->remove_item(19, ($amount_of_new_basic_nets * 3), $user_id);
      for($i=0; ($amount_of_new_basic_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(7, 1, $user_id);
      }
    }

      // Hunter (visible)
    $broken_hunter_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 20 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_hunter_nets = 0;
    if($broken_hunter_nets >= 3) :
      for($i=1; $broken_hunter_nets >= $i; $i++){
        if( is_integer( $i/3 ) ){
          $amount_of_new_hunter_nets++;
        }
      };
    endif;

    if($amount_of_new_hunter_nets > 0){
      $this->user_engine->remove_item(20, ($amount_of_new_hunter_nets * 3), $user_id);
      for($i=0; ($amount_of_new_hunter_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(8, 1, $user_id);
      }
    }

      // Pro (visible)
    $broken_pro_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 21 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_pro_nets = 0;
    if($broken_pro_nets >= 3) :
    ...
    if($amount_of_new_pro_nets > 0){
      $this->user_engine->remove_item(21, ($amount_of_new_pro_nets * 3), $user_id);
      for($i=0; ($amount_of_new_pro_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(9, 1, $user_id);
      }
    }

  // ultimate (visible)
    $broken_ultimate_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 22 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_ultimate_nets = 0;
    if($broken_ultimate_nets >= 3) :
      for($i=1; $broken_ultimate_nets >= $i; $i++){
        if( is_integer( $i/3 ) ){
          $amount_of_new_ultimate_nets++;
        }
      };
    endif;

    if($amount_of_new_ultimate_nets > 0){
      $this->user_engine->remove_item(22, ($amount_of_new_ultimate_nets * 3), $user_id);
      for($i=0; ($amount_of_new_ultimate_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(10, 1, $user_id);
      }
    }
    // god (visible)
    $broken_god_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 23 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_god_nets = 0;
    if($broken_god_nets >= 2) :
      for($i=1; $broken_god_nets >= $i; $i++){
        if( is_integer( $i/2 ) ){
          $amount_of_new_god_nets++;
        }
      };
    endif;

    if($amount_of_new_god_nets > 0){
      $this->user_engine->remove_item(23, ($amount_of_new_god_nets * 2), $user_id);
      for($i=0; ($amount_of_new_god_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(11, 1, $user_id);
      }
    }
// infinite (visible)
    $broken_infinite_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 24 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_infinite_nets = 0;
    if($broken_infinite_nets >= 2) :
      for($i=1; $broken_infinite_nets >= $i; $i++){
        if( is_integer( $i/2 ) ){
          $amount_of_new_infinite_nets++;
        }
      };
    endif;

    if($amount_of_new_infinite_nets > 0){
      $this->user_engine->remove_item(24, ($amount_of_new_infinite_nets * 2), $user_id);
      for($i=0; ($amount_of_new_infinite_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(12, 1, $user_id);
      }
    }
 // Basic (invisible)
    $broken_inbasic_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 25 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_inbasic_nets = 0;
    if($broken_inbasic_nets >= 3) :
      for($i=1; $broken_inbasic_nets >= $i; $i++){
        if( is_integer( $i/3 ) ){
          $amount_of_new_inbasic_nets++;
        }
      };
    endif;

    if($amount_of_new_inbasic_nets > 0){
      $this->user_engine->remove_item(25, ($amount_of_new_inbasic_nets * 3), $user_id);
      for($i=0; ($amount_of_new_inbasic_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(13, 1, $user_id);
      }
    }

      // Hunter (invisible)
    $broken_inhunter_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 26 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_inhunter_nets = 0;
    if($broken_inhunter_nets >= 3) :
      for($i=1; $broken_inhunter_nets >= $i; $i++){
        if( is_integer( $i/3 ) ){
          $amount_of_new_inhunter_nets++;
        }
      };
    endif;

    if($amount_of_new_inhunter_nets > 0){
      $this->user_engine->remove_item(26, ($amount_of_new_inhunter_nets * 3), $user_id);
      for($i=0; ($amount_of_new_inhunter_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(14, 1, $user_id);
      }
    }

      // Pro (invisible)
    $broken_inpro_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 27 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_inpro_nets = 0;
    if($broken_inpro_nets >= 3) :
      for($i=1; $broken_inpro_nets >= $i; $i++){
        if( is_integer( $i/3 ) ){
          $amount_of_new_inpro_nets++;
        }
      };
    endif;

    if($amount_of_new_inpro_nets > 0){
      $this->user_engine->remove_item(27, ($amount_of_new_inpro_nets * 3), $user_id);
      for($i=0; ($amount_of_new_inpro_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(15, 1, $user_id);
      }
    }

  // ultimate (invisible)
    $broken_inultimate_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 28 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_inultimate_nets = 0;
    if($broken_inultimate_nets >= 3) :
      for($i=1; $broken_inultimate_nets >= $i; $i++){
        if( is_integer( $i/3 ) ){
          $amount_of_new_inultimate_nets++;
        }
      };
    endif;

    if($amount_of_new_inultimate_nets > 0){
      $this->user_engine->remove_item(28, ($amount_of_new_inultimate_nets * 3), $user_id);
      for($i=0; ($amount_of_new_inultimate_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(16, 1, $user_id);
      }
    }
    // god (invisible)
    $broken_ingod_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 29 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_ingod_nets = 0;
    if($broken_ingod_nets >= 2) :
      for($i=1; $broken_ingod_nets >= $i; $i++){
        if( is_integer( $i/2 ) ){
          $amount_of_new_ingod_nets++;
        }
      };
    endif;

    if($amount_of_new_ingod_nets > 0){
      $this->user_engine->remove_item(29, ($amount_of_new_ingod_nets * 2), $user_id);
      for($i=0; ($amount_of_new_ingod_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(17, 1, $user_id);
      }
    }
// infinite (invisible)
    $broken_ininfinite_nets = $this->db->query('SELECT item_id, COUNT(id) as amount FROM user_items WHERE item_id = 30 AND soft_deleted = 0 AND user_id = '.$user_id)->row()->amount;
    $amount_of_new_ininfinite_nets = 0;
    if($broken_ininfinite_nets >= 2) :
      for($i=1; $broken_ininfinite_nets >= $i; $i++){
        if( is_integer( $i/2 ) ){
          $amount_of_new_ininfinite_nets++;
        }
      };
    endif;

    if($amount_of_new_ininfinite_nets > 0){
      $this->user_engine->remove_item(30, ($amount_of_new_ininfinite_nets * 2), $user_id);
      for($i=0; ($amount_of_new_ininfinite_nets > $i) AND ($i < 25); $i++){
        $this->user_engine->add_item(18, 1, $user_id);
      }
    }
    ...
    if($this->input->is_ajax_request()):
      ...
      $message = "";
      if($amount_of_new_basic_nets > 0 || $amount_of_new_hunter_nets > 0 || $amount_of_new_pro_nets > 0 || $amount_of_new_ultimate_nets > 0 || $amount_of_new_god_nets > 0 || $amount_of_new_infinite_nets > 0 || $amount_of_new_inbasic_nets > 0 || $amount_of_new_inhunter_nets > 0 || $amount_of_new_inpro_nets > 0 || $amount_of_new_inultimate_nets > 0 || $amount_of_new_ingod_nets > 0 || $amount_of_new_ininfinite_nets > 0){
        $message = 'You now have '.$amount_of_new_basic_nets + $amount_of_new_hunter_nets + $amount_of_new_pro_nets + $amount_of_new_ultimate_nets + $amount_of_new_god_nets + $amount_of_new_infinite_nets + $amount_of_new_inbasic_nets + $amount_of_new_inhunter_nets + $amount_of_new_inpro_nets + $amount_of_new_inultimate_nets + $amount_of_new_ingod_nets + $amount_of_new_ininfinite_nets.' more net(s)!';
      }
      if($amount_of_new_basic_nets > 0 || $amount_of_new_hunter_nets > 0 || $amount_of_new_pro_nets > 0 || $amount_of_new_ultimate_nets > 0 || $amount_of_new_god_nets > 0 || $amount_of_new_infinite_nets > 0 || $amount_of_new_inbasic_nets > 0 || $amount_of_new_inhunter_nets > 0 || $amount_of_new_inpro_nets > 0 || $amount_of_new_inultimate_nets > 0 || $amount_of_new_ingod_nets > 0 || $amount_of_new_ininfinite_nets > 0){
        $reply['message'] = $message;
      }else{
        $reply['message'] = "Nothing to fix! Remember you need 3 broken nets to fix most nets!";
      }
   ...
   */
    ...
    private function _sync_energy($forester_data = array())
    {
        ...
        } else if ($bonus_energy < 0) {
            $this->db->update('forest_users', array('energize_at' => time()), array('user_id' => $forester_data['user_id']));
        }
