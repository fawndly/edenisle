...
class Search extends CI_Controller
{
    ...
    * @return JSON array of results USERS*/
    public function get_items($search_key = '', $search_type = SearchType::ITEMS) {
        ...
        switch ($search_type):
            ...
            case SearchType::USERNAME:
                $data['results'] = $this->_search_username($search_key);
                    foreach ($data['results'] as $key => $username):
                    $data['results'][$key]['user_id'];
                endforeach;
            break;
    ...
    private function _search_username($key) {
        return $this->db->select('user_id, username, user_level, last_action')
            ->like('username', $key)
            ->get('username', self::$SEARCH_LIMIT)->result_array();
    }
