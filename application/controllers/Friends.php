...
class Friends extends CI_Controller
{
    ...
    public function index() {
        ...
        $this->system->quick_parse('friends/index', [
    ...
    public function add_friend() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $friend_query = $this->db->get_where('friends', [
            ...
            if ($friend_query->num_rows() == 0) {
                ...
                if ($friend_data['active'] == 1)
                    show_error('You two are already friends!');

                if ($friend_data['active'] != 2) {
                    $yin = ['user_id' => $friend_data['user_id'], 'friend_id' => $friend_data['friend_id']];
                    $yan = ['user_id' => $friend_data['friend_id'], 'friend_id' => $friend_data['user_id']];
                    ...
                    $this->db->where($yin)->update('friends', $update);
                    $this->db->where($yan)->update('friends', $update);
                }
            }
            ...
            $this->system->quick_parse('friends/add_friend', [
    ...
    public function accept_request() {
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (!is_numeric($friendship_id))
                show_error('user_id must be valid');

            $friend_query = $this->db->select('users.username, friends.*')
                                     ->join('users', 'users.user_id = friends.friend_id')
                                     ->get_where('friends', ['friend_id' => $user_id, 'friendship_id' => $friendship_id]);
            ...
            $this->db->where($yin)->update('friends', $status);
            $this->db->where($yan)->update('friends', $status);
    ...
    public function ignore_request($friendship_id = 0) {
        ...
        $friend_query = $this->db->get_where('friends', ['friendship_id' => $friendship_id]);
        ...
        $yin    = ['user_id' => $friend_data['user_id'], 'friend_id' => $friend_data['friend_id']];
        $yan    = ['user_id' => $friend_data['friend_id'], 'friend_id' => $friend_data['user_id']];
        $update = ['active' => 0, 'unfriend_count' => $friend_data['unfriend_count'] + 1];
    ...
    public function remove_friend() {
        ...
        if (!is_numeric($friend_id))
            show_error('friend_id must be valid');

        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            $friend_query = $this->db->get_where('friends', ['friend_id' => $friend_id, 'user_id' => $this->system->userdata['user_id']]);
            ...
            $yin    = ['user_id' => $friend_data['user_id'], 'friend_id' => $friend_data['friend_id']];
            $yan    = ['user_id' => $friend_data['friend_id'], 'friend_id' => $friend_data['user_id']];
            $update = ['active' => 0, 'unfriend_count' => 1];
