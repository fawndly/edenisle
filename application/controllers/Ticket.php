...
class Ticket extends CI_Controller
{
    ...
    public function index() {
        if (!$this->session->user_id) redirect('signin');
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST" && strlen($this->input->post('description')) > 0) {
            ...
            foreach (array($this->system->userdata['user_id']) as $user_id) {
                $this->db->set('date', 'NOW()', false)->insert('mail', array(
                    ...
                    'included_users' => json_encode(array($this->system->userdata['user_id'] => $this->system->userdata['username'], 2011 => 'Miner Mole Pixel')),
                    'unique_mail_id' => $unique_mail_id,
                    'message'  => 'Hello there! 
                    Your ticket #'.$this->db->insert_id().' with the subject of "'.$this->input->post('issue').'" has been created at '.date("F j, Y, g:i a", time()).'. 
                    Someone from the staff members will attend to your ticket shortly!',
                ));
            ...
            $staff_members = $this->db->get_where('users', array('user_level !=' => 'user', 'user_level !=' => 'verify'))->result_array();
