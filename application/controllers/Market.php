...
class Market extends CI_Controller
{
    ...
    private $simoltanous_item_limit = 8;
    private $listing_fee_percent = 0;
    private $bid_cooloff_seconds = 10; # delays the end time to prevent bid sniping
    ...
    public function __construct()
    {
        ...
        # realtime setup for bidding
        $this->load->library(array('realtime'));
        $realtime_channel = "market";
        $this->realtime->setChannel($realtime_channel);
        $this->realtime_config = [
            "host" => $this->realtime->host,
            "port" => $this->realtime->read_port,
            "token" => $this->realtime->public_app_token,
        ...
        $this->system->view_data['scripts'][] = 'https://js.pusher.com/3.2/pusher.min.js';
        $this->system->view_data['scripts'][] = '/global/js/market.js?7';
        if (!$this->session->user_id) redirect('/auth/signin?r=market');

        if (!$this->cache->get('market_process_tick_cache')) $this->_close_expired_offers();
    }
    ...
    public function index()
    {
        $view_data = [
            ...
            'realtime_setup' => $this->realtime_config,
    ...
    public function purchase_item()
    {
        ...
        if($offer_data['purchased'] == 1 || $offer_data['cancled'] == 1) redirect('market/?just_missed_it=1');
    ...
    public function bid_on_item()
    {
        ...
        # non-duplicate inserts
        $insert_query = $this->db->insert_string('marketplace_watchlist', [
            'user_id' => $this->system->userdata['user_id'],
            'listing_id' => $offer_data['id']
        ]);
        $this->db->query(str_replace('INSERT INTO','INSERT IGNORE INTO', $insert_query));
        ...
        # will auction end within the cooloff timeframe?
        $unix_time_finishes_at = strtotime($offer_data['finishes_at']);
        if (time() > $unix_time_finishes_at - $this->bid_cooloff_seconds) {
          $updated_end_time = date("Y-m-d H:i:s", time() + $this->bid_cooloff_seconds);
          $this->db->set('finishes_at', $updated_end_time, NULL);
        }

        $this->db->update('marketplace_items', $marketplace_items);

        $this->realtime->publish('new_bid', [
            'offer_id' => $offer_id,
            'price' => $this->input->post('offer_price'),
            'min_bid' => $this->input->post('offer_price') + $offer_data['bid_increment'],
            'bidder_username' => $this->system->userdata['username'],
            'new_countdown_seconds' => isset($updated_end_time) ? $this->bid_cooloff_seconds : NULL,
            'last_bidder_id' => $this->system->userdata['user_id']
        ]);
    ...
    public function post_sell() {
        ...
        if( ! is_numeric($this->input->post('ore_price')) || $this->input->post('ore_price') < 1) show_error('price must be a valid number');
        ...
        if ($this->input->post('listing_type') == 'bid') {
            ...
            $this->db->set('published_at', 'NOW()', false)->insert('marketplace_items', [
                ...
                'bid_increment' => $this->input->post('min_bid_increase') ?: 1,
    ...
    private function _marketplace_items($where_conditions = array(), $search_params = array())
    {
        ...
        if (isset($search_params['offer_type']) && strlen($search_params['offer_type']) > 1) {
            if ($search_params['offer_type'] == 'previously_bid' && count($this->_watchlist_listing_ids()) > 0) {
                $this->db->where_in('id', $this->_watchlist_listing_ids());
            } else {
    ...
    private function _watchlist_listing_ids()
    {
        $watchlist_items = $this->db->get_where('marketplace_watchlist', ['user_id' => $this->system->userdata['user_id']])->result_array();

        $extract_listing_id = function($watch){ return $watch['listing_id']; };
        return array_map($extract_listing_id, $watchlist_items);
    }
    ...
    {
        $this->cache->save('market_process_tick_cache', TRUE, 2);
        $this->db->trans_start();
                ...
                try {
                    ...
                    $this->_send_inbox(
                        ...
                        sprintf(
                            ...
                            $offer_data["username"],
                            $offer_data["item_name"],
                            urlencode($bidder_info["username"]),
                    ...
                    $this->_send_inbox(
                        $bidder_info["user_id"],
                        sprintf(
                            $this->lang->line("market_won_message"),
                            $bidder_info["username"],
                            $offer_data["item_name"]
                        )
                    );
