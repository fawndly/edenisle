...
class Forum extends CI_Controller
{
    ...
    function __construct() {
        parent::__construct();
        $this->forum_config = $this->system->get_feature_config('forum'); //@TODO: returning nothing
    ...
    public function index() {
        $this->system->view_data['scripts'][] = '/global/js/forum/index.js?1';
        ...
        if (!$view_data = $this->cache->get('forum_categories')) {
            ...
            $view_data = [
                ...
                'navigation_header' => 'Sapherna\'s forums'
            ];

            $this->cache->save('forum_categories', $view_data, 60);
        }

        $view_data['users_online'] = $this->db->select('username, user_level, last_action as last_activity')
            ->order_by('username', 'asc')
            ->where('users.last_action >= ' . (time() - 1080))
            ->group_by('username')
            ->get('users')
            ->result_array();

        $view_data['users_posting'] = $this->db->select('username, author_id, user_level')
                                               ->join('users', 'users.user_id = topic_posts.author_id')
                                               ->where('NOW() - topic_posts.post_time < 1080')
                                               ->group_by('username')
                                               ->order_by('username', 'asc')
                                               ->get('topic_posts')
                                               ->result_array();

        $view_data['recent_users'] = $this->db->select('username')
            ->order_by('username', 'asc')
            ->where('DATE(users.register_date) = CURDATE() AND users.banned = 0')
            ->get('users')
            ->result_array();
    ...
    public function your_topics() {
        if (!$this->session->userdata('user_id'))
            redirect('auth/signin?r=/forum/your_topics');

        if (!$total_topics = $this->cache->get('user:' .$this->system->userdata['user_id'] . '_topic_post_counter')) {
            $total_topics = $this->db->select('COUNT(1) as total')
                                     ->get_where('topics', array('topics.topic_author' => $this->system->userdata['user_id']))
                                     ->row()
                                     ->total;
        ...
        $config = array(
            'base_url'    => base_url() . 'forum/your_topics/',
            'total_rows'  => $total_topics,
            'per_page'    => 12,
            'uri_segment' => 3
        );
        ...
        $this->system->quick_parse('forum/your_topics', array(
   ...
   **/
    public function bookmarked() {
        if (!$this->session->userdata('user_id'))
            redirect('auth/signin?r=/forum/bookmarked');
     ...
     **/
    function view($id = 0) {
        $data = $this->forum_engine->get_forum_data($id);

        if (!$data) {
            show_404('forum');
        } else if($data['staff'] == 1) {
        ...
        $this->load->library('pagination');
        $config = [
            'base_url'    => base_url() . 'forum/view/' . $id,
            'total_rows'  => $data['total_topics'],
            'per_page'    => 12,
            'uri_segment' => 4
        ];
        $this->pagination->initialize($config);

        $topics = $this->forum_engine->get_forum_topics($id, $this->uri->segment(4, 0), $config['per_page']);

        if ($this->system->is_staff())
            $this->system->view_data['scripts'][] = '/global/js/crysandrea.staff.scripts.js';
                $user_id = $this->session->userdata('user_id');
                $token = ($user_id ? $this->generate_drop($user_id) : NULL );       
        $this->system->quick_parse('forum/view', array(
            'page_title' => $data['forum_name'],
            'page_body'  => 'forums ' . $data['forum_name'],
            'forum_id'   => $id,
            'topics'     => $topics,
            'token' => $token
        ));
    ...
    public function view_topic($id_string = 0) {
        ...
        $this->system->view_data['scripts'][] = '/global/js/forum/view_topic.js?1478789422';
        ...
        if ($this->session->userdata('user_id')) {
            $subscribed = $this->db->limit(1)->select('subscribed_topics')
   ...
   */
    public function edit_post($post_id = 0) {
        if (!is_numeric($post_id))
            show_error('post_id must be valid');
    ...
    if($first_post == TRUE):
      ...
      $this->form_validation->set_rules('title', 'topic title', 'max_length[50]|required');
    else:
      $this->form_validation->set_rules('message', 'message', 'required');
    endif;
      ...
      $this->db->set('post_time', 'NOW()', false)->insert('topic_post_previous_edits', $previous_edit);
        ...
        // if($topic_data['Ores_enabled'] == 1):
        //   $shaved_chars = (strlen($post_data['post_body'])-strlen($this->input->post('message')));
        //  if($shaved_chars > 0):
        //    $this->user_engine->remove('user_Ores', parse_earned_Ores($shaved_chars));
        //endif;
        //endif;
    ...
    public function new_topic($forum_id = 0) {
        ...
        if ($this->cache->get('topic_limit:' . $this->system->userdata['user_id']))
            show_error('Wow... Slow down a little! You must wait at least 30 seconds before creating a new topic');
        ...
        if ($forum_data['staff'] == 1 || $forum_id == 1)
            if (!$this->system->is_staff())
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $Ores = 0;
            $Ores = parse_earned_Ores($this->input->post('message'));
            $this->user_engine->add('user_Ores', $Ores);

            $this->db->query("UPDATE users SET last_action ='".time()."' WHERE user_id ='".$this->session->userdata('user_id')."'");    
            ...
            $this->notification->broadcast(array(
                ...
                'staff_topic'       => ($forum_data['staff']) ? true : false,
            ), TRUE);
    ...
    public function topic_reply($topic_id = 0) {
        ...
        $this->form_validation->set_rules('message', 'Message', 'required|addslashes');

        if ($topic_data == FALSE) {
            show_404('forum');
        } else if($topic_data['staff'] == 1) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $Ores = 0;
            if ($topic_data['Ores_enabled'] == 1) {
                $Ores = parse_earned_Ores($message);
                $this->user_engine->add('user_Ores', $Ores);
                $this->user_engine->add('special_currency', 1); //by VJC
                $this->db->query("UPDATE users SET last_action ='".time()."' WHERE user_id ='".$this->session->userdata('user_id')."'");  
            } else if($topic_data['forum_id'] == 10) {
                // $this->user_engine->add('user_Ores', 1);
            ...
            if (isset($replies[0]) && count($replies[0]) > 0) {
                ...
                foreach ($reply_users->result_array() as $user) {
                    ...
                    $this->notification->yuuNotifyUser([
                        ...
                        'time'      => $post_time,
                        'type'      => 1,
                        'target'    => "/topic/view/{$topic_id}/{$page_id}#{$last_id}",
                    ]);
                }
            }
            ...
            $this->output->set_content_type('application/json')->set_output(json_encode(validation_errors(), JSON_NUMERIC_CHECK));
        }
    }
  ...
  public function toggle_bookmark($topic_id = 0)
  {
   ...
   {
       $this->system->quick_parse('forum/bbcode', $view_data);
   }
