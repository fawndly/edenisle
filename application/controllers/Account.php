...
class Account extends CI_Controller
{
    ...
    var $route_navigation = array(
        ...
        'preferences' => array(
            'icon'        => '/global/css/images/icons/equalizer.png',
            'title'       => 'Preferences',
            'description' => 'Here\'s you can change some settings to customize the site for you.',
            'enabled'     => false
        ),
        'profile' => array(
            'icon'        => '/global/css/images/icons/equalizer.png',
        ...
        'signature' => array(
            ...
            'description' => 'Your signature and title gives you room for something you\'d like to share.',
            'enabled'     => TRUE
        ),
        'privacy' => array(
            'icon'        => '/global/css/images/icons/equalizer.png',
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() === TRUE):
            if($changed_username && $this->system->userdata['user_Ores'] >= 1000):
                $success_notices[] = 'Your username has been successfully changed to "'.$this->input->post('username').'" for 1,000 Ores.';
                
                $this->user_engine->remove('user_Ores', 1000);
     ...
     * Preferences
     *
     * Allow users to edit some account preferences
     *
     * @access  public
     * @param   none
     **/
    function preferences()
    {
        ...
        $data = array(
            ...
            'page_body'  => 'account sub-preferences',
            'routes'     => $this->route_navigation,
            'active_url' => $this->uri->rsegment(2, 0),
            'success'    => $success_notices,
        );

        $this->system->quick_parse('account/preferences', $data);
   ...
   /**
    ...
    public function profile()
    {
        ...
        $this->form_validation->set_rules('likes', 'Liked', 'htmlentities|addslashes');
        $this->form_validation->set_rules('dislikes', 'Dislikes', 'htmlentities|addslashes');
        $this->form_validation->set_rules('hobbies', 'Hobbies', 'htmlentities|addslashes');
        $this->form_validation->set_rules('profile_bio', 'Biography', 'htmlentities');
        $this->form_validation->set_rules('profile_css', 'Profile CSS', 'addslashes');
    ...
    function signature()
    {
        ...
        $success_notices = array();
        $changed_title = ($this->input->post('user_title') != $this->system->userdata['user_title']);

        $this->form_validation->set_rules('user_signature', 'Signature', 'htmlentities|addslashes');

        if ($this->form_validation->run() === TRUE):
            if($changed_title && $this->system->userdata['user_Ores'] >= 100):
                $success_notices[] = 'Your title has been successfully changed to "'.$this->input->post('user_title').'" for 100 currency.';
                
                $this->user_engine->remove('user_Ores', 100);
                $this->db->insert('user_title', $this->input->post('user_title'));
            endif;
