...
class Avatar_engine extends CI_Model
{
    ...
    function get_user_inventory($user_id = 0)
    {
        ...
        $sql = "SELECT cai.name as itemname,
                        ...
                        al.id as layer_id,
                        CASE WHEN alt.tab_id IS NOT NULL THEN alt.tab_name ELSE avatar_layer_tabs.tab_name END AS tab,
                        al.main_tab,
                        caui.id as main_id,
                ...
                LEFT JOIN avatar_layer_tabs alt ON alt.tab_id = cai.tab_id
                GROUP BY item_grouper
                HAVING item_identifier = 0
                ORDER BY alt.tab_id, al.order ASC, cai.name ASC";
