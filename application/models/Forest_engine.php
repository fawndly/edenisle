...
class Forest_engine extends CI_Model
{
    ...
    function add_palladium($_user_id, $_amount){
        $query = $this->db->query("SELECT user_Ores FROM users WHERE user_id = ".$_user_id."")->row();

        $new_palladium = $query->user_Ores + $_amount;

        $this->db->where('user_id', $_user_id)->update('users', array('user_Ores' => $new_palladium));
    ...
    $net .= '<div class="bug_shelf">
            ...
            <strong>Price:</strong> '.$insect['price'].' Ores<br />
