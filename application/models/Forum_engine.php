...
class Forum_engine extends CI_Model
{
    ...
    function get_forum_data($id) {
        ...
        $forum_data = $this->db->select('forum_name, forum_description, Ores_enabled, user_post, forums.staff, parent_id, categories.name')
                               ->from('forums')
    ...
    function get_forum_topics($id, $limit, $start) {
        if (empty($id))
            $id = $this->forum_id($id); // ID assumption
     ...
     * @return  object
     */
    function create_topic($data = array()) {
        if (isset($data['forum_id'])) 
            $data['forum_id'] = $this->forum_id;

        if (isset($data['topic_type']) && !$data['topic_type'])
    ...
    function get_topic_data($id = 0, $count_posts = TRUE) {
        ...
        $topic_data = $this->db->select('topics.*, forums.staff as staff, forums.forum_name, forums.short_name, forums.Ores_enabled')
     ...
     * @route   n/a
     */
    function log_prevous_edit($post_data = []) {
        $insert_array = [
    ...
    function make_unsticky($topic_id) {
        $this->db->update('topics', array('topic_type' => null), array('topic_id' => $topic_id));
    }
    ...
    function delete_post($post_data, $topic_data) {
        ...
        $new_topic_data = ['total_posts' => $topic_data['total_posts']-1];
        $this->db->where('topic_id', $topic_data['topic_id']);
        $this->db->update('topics', $new_topic_data);
