...
if ( ! function_exists('notice'))
{
    function notice($header = '', $string = '', $class = 'success')
    {
        switch($class){
            case "success":
                return '<div class="success"><h3>'.image('images/icons/yay.png').' '.$header.'</h3><p>'.$string.'</p></div>';
            break;
            case "error":
                return '<div class="error"><h3>'.image('images/icons/stop.png').' '.$header.'</h3><p>'.$string.'</p></div>';
            break;
            case "notice":
                return '<div class="notice"><h3>'.image('images/icons/woah.png').' '.$header.'</h3><p>'.$string.'</p></div>';
            break;
        }
    }
}
...
function _datadate($date){
    return human_time($date).' (<small>'.date("M jS, Y H:i", strtotime($date)).'</small>)';
    // if(time()-86400 > strtotime($date)){
        // return human_time($date).' (<small>'.date("M jS, Y H:i:s", strtotime($date)).'</small>)';
    // } else {
...
function user_color($id) {
    $rank = [
        ...
        'developer' => "#A362CC",
        'pixelist' => "#5224D1",
        'pixelist_assistant' => "#A199F7",
        'digital_artist' => "#F7ADCB",
        'event_manager' => "#700945",
        'storyliner' => "#4E8F46",
        'technical_support' => "#C96D6D",
        'moderator' => "#74D1D6",
    ];

    return isset($rank[$id]) ? $rank[$id] : "#000";
}
...
{
    function lazy_item_thumb($src = '', $attributes = '')
    {
        return image('/images/icons/large_loader.gif', $attributes = $attributes.' class="lazy" width="45" height="45" data-src="'.$src.'"');
    }
}
