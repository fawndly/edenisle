<!-- Crysandrea engine by Tyler Diaz (https://github.com/Pixeltweak/Crysandrea) --><?php $this->load->view('trades/trades_navigation'); ?>
...
<script type="text/javascript">
    var user_role = "<?= $role ?>",
        inventory_keys = <?= $item_names ?>,
        trade_id = <?= $trade_id ?>,
        inventory_match = <?= $item_data ?>;
</script>
...
<div class="clearfix" style="position:relative;">
    <div class="trade_container" style="margin-right:6px" id="sender_container">
        <div class="tc_header">
            <img src="/images/avatars/<?= $trade['trade_sender'] ?>_headshot.png" class="avatar xflip" alt="" />
            <div class="tc_user_box">
                <strong><?= anchor('/user/'.urlencode($trade['sender']->user->username), $trade['sender']->user->username) ?></strong><br />
                <span>Trade status: <?= $trade['sender_status_text'] ?></span>
            </div>
        </div>
        <div class="tc_box">
            <div class="tc_currencies row">
                <div class="currency palladium span3">
                    <span>Ores:</span>
                    <strong><?= number_format($trade['sender_ores']) ?></strong>
                </div>
                <div class="currency berries span3">
                    <span>Berries:</span>
                    <strong><?= number_format($trade['sender_berries']) ?></strong>
                </div>
            </div>
            <div class="tc_items">
            ...
            <?php if ($trade['receiver_status'] != 0): ?>
                <div class="pull-right">
                    <?php if ($trade['sender_status'] == 0): ?>
                        <form action="/trades/accept_trade/<?= $trade_id ?>" method="POST" style="display:inline">
                    ...
                    <?php endif ?>
                    or
                    <form action="/trades/accept_trade/<?= $trade_id ?>/cancel" method="POST" style="display:inline">
    ...
    <div class="trade_container" id="receiver_container">
        <div class="tc_header">
            <img src="/images/avatars/<?= $trade['trade_receiver'] ?>_headshot.png" class="avatar" style="float:right; margin-left:5px;" alt="" />
            <div class="tc_user_box pull-right" style="text-align:right; float:right;">
                <strong><?= anchor('/user/'.urlencode($trade['receiver']->user->username), $trade['receiver']->user->username) ?></strong><br />
                <span>Trade status: <?= $trade['receiver_status_text'] ?></span>
            </div>
        </div>
        <div class="tc_box">
            <div class="tc_currencies">
                <div class="tc_currencies row">
                    <div class="currency palladium span3">
                        <span>Ores:</span>
                        <strong><?= number_format($trade['receiver_ores']) ?></strong>
                    </div>
                    <div class="currency berries span3">
                        <span>Berries:</span>
                        <strong><?= number_format($trade['receiver_berries']) ?></strong>
                    </div>
                </div>
            </div>
            <div class="tc_items">
...
<?php if (($trade['sender_status'] > 0 && $trade['receiver_status'] > 0) == FALSE): ?>
    ...
    <div class="clearfix">
        <div class="def_structure">
            <div class="def_content">
                ...
                <div class="push_up">
                    <form class="form-search pull-left" id="add_currency" method="POST" action="/trades/add_currency/<?= $trade_id ?>">
            ...
            <div class="def_content">
                ...
                <div class="push_up">
                    <form class="form-search" style="margin-top:3px;" id="search_inventory" method="GET" action="/trades/view_trade/<?= $trade_id ?>">
    ...
    <div class="clearfix">
        ...
        <ul class="nav nav-tabs" id="avatar_items">
            <?php foreach ($items as $tab_name => $tab_array): ?>
        ...
        <div class="tab-content" id="inventory_items">
            <?php foreach ($items as $tab_name => $tab_array): ?>
                <div class="tab-pane" id="<?= strtolower($tab_name) ?>">
                    <?php foreach ($tab_array as $itemGroup): ?>
