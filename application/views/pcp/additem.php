...
<div class="grid_3">
    ...
    <br>
    <div class="well">
      <strong>(<input type="checkbox" id="master-recolor-box" name=""> all) Auto-recolor item:</strong>
      <br />
      <?php foreach ($colors as $color_value): ?>
        <label for="color_<?php echo $color_value['name'] ?>">
          <input type="checkbox" name="item[colors][<?php echo $color_value['id'] ?>]" class="recolor-box" id="color_<?php echo $color_value['name'] ?>">
          (#<?php echo $color_value['id'] ?>) <?php echo $color_value['name'] ?>
        </label>
      <?php endforeach ?>
    </div>
                  ...
                  <td>
                      <select name="part[layer][]">
                          <?php foreach($layers as $k => $v): ?>
                               <option <?php if($v['id'] == $part_layer_value[$i]) {?> selected="selected" <?php } ?> value="<?=$v['id'];?>"><?=str_pad($k, 3, '0', STR_PAD_LEFT)?> - <?=$v['name'];?></option>
                          <?php endforeach; ?>
                      </select>
                  </td>
              </tr>
